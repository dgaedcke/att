...
ALTER TABLE `att_attribute` 
	CHANGE `att_encode` `att_encode` tinyint(3) unsigned   NOT NULL DEFAULT '0' COMMENT 'xtra bits 1 = private/company specific attribute' after `att_is_long`, COMMENT='';
...
/* Alter table in target */
ALTER TABLE `val_value` 
	CHANGE `val_id` `val_id` bigint(20) unsigned   NOT NULL auto_increment COMMENT 'rid' first, 
	CHANGE `val_ent_id` `val_ent_id` bigint(20) unsigned   NOT NULL DEFAULT '0' COMMENT 'entity id' after `val_id`, 
	CHANGE `val_dom_id` `val_dom_id` smallint(10) unsigned   NOT NULL DEFAULT '0' COMMENT 'domain id' after `val_ent_id`, 
	CHANGE `val_att_id` `val_att_id` mediumint(10) unsigned   NOT NULL DEFAULT '0' COMMENT 'attribute id' after `val_dom_id`, 
	CHANGE `val_version` `val_version` smallint(5) unsigned   NOT NULL DEFAULT '1' COMMENT 'value version  niu currently' after `val_att_id`, 
	CHANGE `val_mod_dttm` `val_mod_dttm` timestamp   NOT NULL DEFAULT CURRENT_TIMESTAMP  on update CURRENT_TIMESTAMP COMMENT 'date time when value last changed' after `val_encode`, 
	CHANGE `val_value` `val_value` varchar(140)  COLLATE utf8_general_ci NOT NULL COMMENT 'value stored or -1 if > 140 chars' after `val_mod_dttm`, COMMENT='';

/* Alter table in target */
ALTER TABLE `var_value_archive` 
	CHANGE `var_mod_dttm` `var_mod_dttm` timestamp   NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT 'date time when this value was originally stored' after `var_encode`, COMMENT='';
/*  Create View in target  */

DELIMITER $$
create view `v10000` as select 1 AS `n` from (((`att`.`v10` `a` join `att`.`v10` `b`) join `att`.`v10` `c`) join `att`.`v10` `d`)$$
DELIMITER ;

/* Create Procedure in target  */

DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `createEntity`(IN p_ent_type VARCHAR(40), IN p_xxx_id BIGINT, IN p_com_id int, IN p_encode TINYINT, inout p_ent_id bigint)
    MODIFIES SQL DATA
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `listAtts`(in p_dom_name varchar(40), IN p_com_id INT, In p_encode tinyint)
    READS SQL DATA
    COMMENT 'list atts by global or company'
BEGIN
/*	call listAtts('preferences',14, 0);
	p_dom_name is currently being ignored
*/
	SELECT att_id, att_dty_id, att_in_all_dom, att_keep_old, att_hash_it, att_is_long, att_name
	FROM att.att_attribute C
	LEFT OUTER JOIN csa_comp_specific_attr S ON S.csa_att_id = C.att_id AND S.csa_com_id = p_com_id
	WHERE C.att_encode&1 = 0 OR S.csa_att_id IS NOT NULL;
END$$
DELIMITER ;
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `loadAttHistory`(in p_ent_id bigint, In p_ent_ety_id int, IN p_ent_com_id INT
...
so vals should come in looking like:  domain_name:attribute_name(comma)dom2:att2
	...
	CALL loadAttHistory(17, 4, 22, 'attribute:age,attribute:dob', 2, 0); -- get age & dob from the domain "attribute"
	or -- with no deliminter in P4
	CALL loadAttHistory(17, 4, 22, 'attribute', 0, 0); -- get ALL vals for this entity from doamin "attribute"
	*/
	...
	if not exists(SELECT 1 FROM att.ent_entity WHERE ent_id = p_ent_id AND ent_com_id = p_ent_com_id AND ent_ety_id = p_ent_ety_id) then
		Select 'Err:  No such entity exists!' as Err;
		leave sproc;
	end if;
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `loadAtts`(in p_ent_id bigint, In p_ent_ety_id int, IN p_ent_com_id INT
	, In p_att_list_or_dom_name varchar(600), In p_att_count smallint, In p_encode tinyint)
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `loadEntity`(in p_ent_type varchar(40), In p_xxx_id bigint, IN p_com_id INT, In p_encode tinyint)
    MODIFIES SQL DATA
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `niu_loadAllAtts`(in p_ent_id bigint, In p_ent_ety_id int
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `raiseError`(IN p_sql_state varchar(8), IN p_err_msg VARCHAR(200), IN p_encode TINYINT)
    NO SQL
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `searchForEntity`(IN p_ent_type VARCHAR(40)
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `spTest`(IN p_encode TINYINT)
    MODIFIES SQL DATA
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `storeAtts`(IN p_ent_id BIGINT, IN p_com_id int, IN p_row_count int, IN p_1 TINYINT, IN p_2 TINYINT, IN p_3 TINYINT, IN p_encode TINYINT)
    MODIFIES SQL DATA
    COMMENT 'store all passed atts'
sproc:BEGIN
...
*/
	...
	-- lookup fkey vals only once & update dom/att ID's in an un-contended (prob-in memory) table
	update _att_stage S
	...
	where 1&ats_encode = 0; -- if caller wants to provide att & dom ID's himself like:
		-- S.ats_dom_name = D.dom_id, S.ats_att_name = A.att_id
		-- we can skip the join/lookup and use the passed table AS IS
	-- after the update above, my ats_???_name cols now contain their respective ID's for the ops below
	...
	IF v_test_mode THEN -- show table before inserts/ updates are run for testing
		Select ats_dom_name, ats_att_name, ats_value, ats_keep_old, ats_is_long, ats_is_multi, ats_hash_it, ats_encode
		from _att_stage; -- test code only
	end if;
	
	-- archive old values if specified & ONLY if value has changed
	INSERT ignore INTO var_value_archive -- ignore for date time conflicts
		(var_val_id, var_value, var_encode, var_mod_dttm)
		SELECT straight_join V.val_id, coalesce(L.lva_value, V.val_value), V.val_encode, V.val_mod_dttm
		FROM _att_stage S
		...
		where S.ats_keep_old -- was set by the caller or settings in dxa_dom_x_att or att_attribute respectively
				-- only create archive rec if value has changed & if old val is not empty
			and (L.lva_val_id IS NULL and V.val_value <> S.ats_value and V.val_value <> '')
				or (L.lva_val_id is not null and L.lva_value <> S.ats_value and L.lva_value <> '');
	
	-- store the current (short) value or -1 if length(value) is > 140
	INSERT INTO val_value
		...
		On duplicate key update val_value = values(val_value)
			, val_mod_dttm = if(S.ats_is_long,now(),val_mod_dttm); -- val_mod_dttm set automatically ONLY if value is new or long
	
	Set v_inst_updt_count = row_count(); -- not accurate due to:
	-- "ON DUPLICATE KEY UPDATE" returning 2; could make result > than p_row_count;  OR
	-- recs may not be updated because vals have not changed & thus count will be low (result < p_row_count)
	...
	-- not accurate due to "ON DUPLICATE KEY UPDATE" returning 2 or same vals resent; lowest value is accurate
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`localhost` PROCEDURE `up_SessInit_Connect`(In pCond Int)
    READS SQL DATA
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`localhost` PROCEDURE `up_Signal_Error`(In pErrCode Int, In pErrMsg VarChar(350), In pCallingProc Char(20)
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`localhost` PROCEDURE `up_SplitStr`(IN pMapKeys Varchar(1600),
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `zadm_createMissingVals`(IN p_encode TINYINT)
    MODIFIES SQL DATA
    COMMENT 'store all passed atts'
BEGIN
/* call zadm_createMissingVals(0);
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` PROCEDURE `zadm_truncTables`(IN p_encode TINYINT)
    MODIFIES SQL DATA
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`localhost` PROCEDURE `zdm_memT100_Init`(In pTotRowsMemT100 Int, In pNoRecursion Bool)
    MODIFIES SQL DATA
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`localhost` FUNCTION `f_User_ShashPW`(pUserSalt Char(2), pPWLiteralString Varchar(32)
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`localhost` FUNCTION `fwwq`(pString Varchar(440), pEncode Tinyint) RETURNS varchar(450) CHARSET latin1
    NO SQL
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` FUNCTION `hashIt`(p_value varchar(140), p_salt VARCHAR(4)) RETURNS varchar(140) CHARSET utf8
    DETERMINISTIC
    COMMENT 'hashed val'
BEGIN
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` FUNCTION `lookupDomainType`(p_dom_name varchar(40), p_encode tinyint) RETURNS int(11)
    DETERMINISTIC
...
DELIMITER $$
CREATE DEFINER=`deweyg`@`%` FUNCTION `lookupEntityType`(p_ety_name varchar(40), p_encode tinyint) RETURNS int(11)
    DETERMINISTIC
